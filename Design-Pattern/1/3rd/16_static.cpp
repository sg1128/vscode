...
// 2. 정적 멤버 데이터 변수는 객체를 만들지 않아도 메모리에 존재
// => 전역변수의 수명과 동일
...
// 정리
// 1. 정적 멤버 데이터
// - 모든 객체가 공유하는 멤버 데이터, 객체를 생성하지 않아도 메모리에 존재
// - 전역변수와 달리 접근 지정자를 사용할 수 있고, 클래스 안에 포함되기 때문에 관련되 데이터를 묶어서 관리 가능
// 2. 정적 멤버 함수
